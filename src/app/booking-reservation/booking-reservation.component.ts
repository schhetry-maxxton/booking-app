...
export class BookingReservationComponent {
  ...
  selectDate(day: number, month: number, year: number): void {
    ...
    const istSelectedDate = this.getISTDate(selectedDate);
    // If no arrival date is selected, check if the selected date is a valid arrival date
    if (!this.selectedArrivalDate && this.validArrivalDates.has(istSelectedDate.toISOString().split('T')[0])) {
      this.selectedArrivalDate = istSelectedDate;
    ...
    else if (this.selectedArrivalDate && istSelectedDate > this.selectedArrivalDate) {
      this.selectedDepartureDate = istSelectedDate;
  ...
  generateValidDepartureDates(): void {
    ...
    this.rooms.forEach(room => {
      room.availabilities.forEach(avail => {
        ...
        // Set the effective range of departure dates
        ...
        // Loop through potential departure dates and validate them
  ...
  formatDate(date: Date): string {
    const istDate = this.getISTDate(date);  // Ensure date is in IST
  ...
  private makeRoomsData(rooms: IRoomWithAvailability[], availabilities: IRoomAvailability[]): IRoomWithAvailability[] {
    ...
    availabilities.forEach(avail => {
      const room = roomMap.get(avail.roomId);
      if (room) {
        room.availabilities.push(avail);
      }
    });
  ...
  //   // If no arrival date is selected, only highlight valid arrival dates
  //   if (!this.selectedArrivalDate) {
  //     if (this.validArrivalDates.has(dateString)) {
  //       return 'available-arrival';  // Valid arrival date
  //     }
  ...
  //   if (this.selectedArrivalDate && !this.selectedDepartureDate) {
  //     // Keep the selected arrival date highlighted
  //     if (currentDate.getTime() === this.selectedArrivalDate.getTime()) {
  //       return 'selected-arrival';  // Keep the arrival date in circular highlight
  //     }

  //     // Only enable valid departure dates
  //     if (this.validDepartureDates.has(dateString)) {
  //       return 'available-departure';  // Valid departure date
  //     }

  //     return 'disabled';  // Disable all other dates
  //   }

  //   // Check if this is the selected departure date
  //   if (this.selectedDepartureDate && currentDate.getTime() === this.selectedDepartureDate.getTime()) {
  //     return 'selected-departure';  // Highlight the selected departure date
  //   }
  ...
  //     return 'selected-range';  // Highlight the range
  //   }

  //   return 'disabled';  // Default: disable everything else
  // }
  ...
  getCellClass(dayObj: { day: number, fromPreviousMonth: boolean }, month: number, year: number): string {
    ...
    // Keep the selected arrival date highlighted (even after selecting departure)
    if (this.selectedArrivalDate && currentDate.getTime() === this.selectedArrivalDate.getTime()) {
      return 'selected-arrival';  // Keep the arrival date highlighted
    }
    ...
    this.rooms.forEach(room => {
      room.availabilities.forEach(avail => {
        ...
        for (let date = new Date(effectiveFromDate); date <= effectiveToDate; date.setDate(date.getDate() + 1)) {
          const dayOfWeek = date.toLocaleDateString('en-US', { weekday: 'short' }).toUpperCase();
          if (arrivalDays.includes(dayOfWeek)) {
  ...
  onArrivalDateSelected(arrivalDate: string) {
    this.selectedArrivalDate = new Date(arrivalDate);
    ...
    this.rooms.forEach(room => {
      room.availabilities.forEach(avail => {
        ...
        for (let date = new Date(effectiveMinDeparture); date <= effectiveMaxDeparture; date.setDate(date.getDate() + 1)) {
          const dayOfWeek = date.toLocaleDateString('en-US', { weekday: 'short' }).toUpperCase();
          if (departureDays.includes(dayOfWeek)) {
            this.validDepartureDates.add(date.toISOString().split('T')[0]);
          }
        }
      });
